...
class BatchResponse(BaseModel):
    id: str
    status: int
    headers: Optional[Dict[str, str]] = Field(exclude=True)
    body: Optional[Any]
...
class BatchIn(BaseModel):
    ...
    def validate_requests(cls, val: List[BatchRequest]) -> List[BatchRequest]:
        print("Num of reqs:", len(set((req.id for req in val))) )
        if len(val) > 1000:
            raise ValueError("Only a maximum of 1000 batch requests are supported")
